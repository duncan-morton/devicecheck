...
const issuePath = '/issues/keyboard-ghosting-windows'
export const metadata: Metadata = { ...genMeta({
  title: 'Keyboard Ghosting on Windows - Complete Fix Guide',
  description: 'Fix keyboard ghosting on windows. Step-by-step troubleshooting guide covering Windows settings, permissions, drivers, and solutions for keyboard ghosting or keys not registering when pressing multiple keys.',
  path: issuePath,
  keywords: ["keyboard ghosting windows","keyboard ghosting fix","keys not registering windows","keyboard rollover issues","windows keyboard ghosting"]
}) }

const faqs = [
  {
    "question": "Why is keyboard ghosting on windows?",
    "answer": "Keyboard ghosting or keys not registering when pressing multiple keys. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix keyboard ghosting on windows?",
  ...
  {
    "question": "What causes keyboard ghosting on windows?",
  ...
  {
    "question": "Can I fix keyboard ghosting on windows without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Keyboard Ghosting on Windows - Complete Fix Guide',
    'Fix keyboard ghosting on windows. Step-by-step troubleshooting guide.',
    '/issues/keyboard-ghosting-windows',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Windows device issues","path":"/hubs/windows-device-issues"},{"name":"Keyboard Ghosting on Windows","path":"/issues/keyboard-ghosting-windows"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/keyboard-ghosting-windows',
    name: 'Keyboard Ghosting on Windows',
    description: 'Fix keyboard ghosting on windows with clear steps for Windows covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Keyboard Ghosting on Windows', path: '/issues/keyboard-ghosting-windows' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Keyboard Ghosting on Windows</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Keyboard ghosting or keys not registering when pressing multiple keys on Windows. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>
          ...
          <IssueLinksPanel issue={{ slug: "keyboard-ghosting-windows", deviceType: "keyboard", platform: "Windows", title: "Keyboard Ghosting on Windows" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Keyboard ghosting or keys not registering when pressing multiple keys"
              platform="Windows"
              deviceType="keyboard"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'keyboard', platform: "Windows", slug: 'keyboard-ghosting-windows' }} />
