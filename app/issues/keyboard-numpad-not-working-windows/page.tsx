...
const faqs = [
  {
    "question": "Why is keyboard numpad not working on windows?",
    "answer": "Keyboard number pad not responding on Windows. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix keyboard numpad not working on windows?",
  ...
  {
    "question": "What causes keyboard numpad not working on windows?",
  ...
  {
    "question": "Can I fix keyboard numpad not working on windows without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Keyboard Numpad Not Working on Windows - Complete Fix Guide',
    'Fix keyboard numpad not working on windows. Step-by-step troubleshooting guide.',
    '/issues/keyboard-numpad-not-working-windows',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Windows device issues","path":"/hubs/windows-device-issues"},{"name":"Keyboard Numpad Not Working on Windows","path":"/issues/keyboard-numpad-not-working-windows"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/keyboard-numpad-not-working-windows',
    name: 'Keyboard Numpad Not Working on Windows',
    description: 'Fix keyboard numpad not working on windows with clear steps for Windows covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Keyboard Numpad Not Working on Windows', path: '/issues/keyboard-numpad-not-working-windows' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Keyboard Numpad Not Working on Windows</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Keyboard number pad not responding on Windows. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>

          <IssueDiagnostic device="keyboard" mode="defer" />

          <IssueLinksPanel issue={{ slug: "keyboard-numpad-not-working-windows", deviceType: "keyboard", platform: "Windows", title: "Keyboard Numpad Not Working on Windows" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Keyboard number pad not responding on Windows"
              platform="Windows"
              deviceType="keyboard"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'keyboard', platform: "Windows", slug: 'keyboard-numpad-not-working-windows' }} />
