...
const faqs = [
  {
    "question": "Why is microphone works in some apps but not others?",
    "answer": "Microphone detected inconsistently across apps. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix microphone works in some apps but not others?",
  ...
  {
    "question": "What causes microphone works in some apps but not others?",
  ...
  {
    "question": "Can I fix microphone works in some apps but not others without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Microphone Works in Some Apps but Not Others - Complete Fix Guide',
    'Fix microphone works in some apps but not others. Step-by-step troubleshooting guide.',
    '/issues/microphone-works-in-some-apps-not-others',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Microphone Works in Some Apps but Not Others","path":"/issues/microphone-works-in-some-apps-not-others"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/microphone-works-in-some-apps-not-others',
    name: 'Microphone Works in Some Apps but Not Others',
    description: 'Fix microphone works in some apps but not others with clear steps for All covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Microphone Works in Some Apps but Not Others', path: '/issues/microphone-works-in-some-apps-not-others' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Microphone Works in Some Apps but Not Others</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Microphone detected inconsistently across apps on All. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Microphone detected inconsistently across apps"
              platform="All"
              deviceType="mic"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'mic', platform: "All", slug: 'microphone-works-in-some-apps-not-others' }} />
