...
const faqs = [
  {
    "question": "Why is chrome microphone permission blocked?",
    "answer": "Chrome blocking microphone access. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix chrome microphone permission blocked?",
    "answer": "First, check Chrome permissions and settings. Verify the correct microphone is selected. Update drivers if needed. Use the microphone test to verify the device is working correctly. Follow the step-by-step guide above for detailed instructions."
  },
  {
  ...
  {
    "question": "What causes chrome microphone permission blocked?",
  ...
  {
    "question": "Can I fix chrome microphone permission blocked without technical knowledge?",
    "answer": "Yes. Most microphone issues can be fixed by checking permissions in Chrome settings and ensuring the correct device is selected. Follow the step-by-step guide above for detailed instructions."
  },
  {
    "question": "How do I enable microphone in Chrome?",
    "answer": "Click the lock icon in Chrome's address bar, then set Microphone to \"Allow\". You can also go to chrome://settings/content/microphone for global settings. Ensure system permissions are also enabled."
  }
]

export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Chrome Microphone Permission Blocked - Complete Fix Guide',
    'Fix chrome microphone permission blocked. Step-by-step troubleshooting guide.',
    '/issues/chrome-microphone-permission-blocked',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Chrome & browser permission issues","path":"/hubs/chrome-permissions-issues"},{"name":"Chrome Microphone Permission Blocked","path":"/issues/chrome-microphone-permission-blocked"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/chrome-microphone-permission-blocked',
    name: 'Chrome Microphone Permission Blocked',
    description: 'Fix chrome microphone permission blocked with clear steps for Chrome covering permissions, device selection, and drivers.',
    steps: [{"title":"Enable microphone in Chrome","description":"Enable microphone in Chrome settings"},{"title":"Check browser permissions","description":"Check browser permissions"},{"title":"Verify Windows microphone access","description":"Verify Windows microphone access"},{"title":"Clear browser cache","description":"Clear browser cache"},{"title":"Update Chrome to latest","description":"Update Chrome to latest version"}]
  })
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Chrome Microphone Permission Blocked', path: '/issues/chrome-microphone-permission-blocked' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Chrome Microphone Permission Blocked</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Chrome blocking microphone access. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>

          <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700">
            This is a Chrome device issue. <Link href="/hubs/chrome-permissions-issues" className="text-blue-600 hover:text-blue-800 font-medium">See all Chrome & browser permission issues →</Link>
          </div>


          <IssueDiagnostic device="mic" mode="defer" />

          <IssueLinksPanel issue={{ slug: "chrome-microphone-permission-blocked", deviceType: "mic", platform: "Chrome", title: "Chrome Microphone Permission Blocked" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Chrome blocking microphone access"
              platform="Chrome"
              deviceType="mic"
            />
            ...
            <ul className="list-disc pl-6 space-y-2 text-gray-700 mb-6">
              <li>Enable microphone in Chrome settings</li>
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'mic', platform: "Chrome", slug: 'chrome-microphone-permission-blocked' }} />
