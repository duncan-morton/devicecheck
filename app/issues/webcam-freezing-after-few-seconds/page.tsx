...
const issuePath = '/issues/webcam-freezing-after-few-seconds'
const alternates = {
...
export const metadata: Metadata = { ...genMeta({
  title: 'Webcam Freezes After a Few Seconds - Complete Fix Guide',
  description: 'Fix webcam freezes after a few seconds. Step-by-step troubleshooting guide covering All settings, permissions, drivers, and solutions for webcam works briefly then freezes.',
  path: issuePath,
  keywords: ["webcam freezing","camera stops after seconds","webcam freeze fix"]
}), alternates }

const faqs = [
  {
    "question": "Why is webcam freezes after a few seconds?",
    "answer": "Webcam works briefly then freezes. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix webcam freezes after a few seconds?",
  ...
  {
    "question": "What causes webcam freezes after a few seconds?",
  ...
  {
    "question": "Can I fix webcam freezes after a few seconds without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Webcam Freezes After a Few Seconds - Complete Fix Guide',
    'Fix webcam freezes after a few seconds. Step-by-step troubleshooting guide.',
    '/issues/webcam-freezing-after-few-seconds',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Webcam Freezes After a Few Seconds","path":"/issues/webcam-freezing-after-few-seconds"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/webcam-freezing-after-few-seconds',
    name: 'Webcam Freezes After a Few Seconds',
    description: 'Fix webcam freezes after a few seconds with clear steps for All covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Webcam Freezes After a Few Seconds', path: '/issues/webcam-freezing-after-few-seconds' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Webcam Freezes After a Few Seconds</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Webcam works briefly then freezes on All. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>

          <IssueDiagnostic device="webcam" mode="defer" />

          <IssueLinksPanel issue={{ slug: "webcam-freezing-after-few-seconds", deviceType: "webcam", platform: "All", title: "Webcam Freezes After a Few Seconds" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Webcam works briefly then freezes"
              platform="All"
              deviceType="webcam"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'webcam', platform: "All", slug: 'webcam-freezing-after-few-seconds' }} />
