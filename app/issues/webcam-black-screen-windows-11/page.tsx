...
const faqs = [
  {
    "question": "Why is webcam shows black screen on windows 11?",
    "answer": "Webcam turns on but only shows a black screen. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix webcam shows black screen on windows 11?",
  ...
  {
    "question": "What causes webcam shows black screen on windows 11?",
  ...
  {
    "question": "Can I fix webcam shows black screen on windows 11 without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Webcam Shows Black Screen on Windows 11 - Complete Fix Guide',
    'Fix webcam shows black screen on windows 11. Step-by-step troubleshooting guide.',
    '/issues/webcam-black-screen-windows-11',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Windows device issues","path":"/hubs/windows-device-issues"},{"name":"Webcam Shows Black Screen on Windows 11","path":"/issues/webcam-black-screen-windows-11"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/webcam-black-screen-windows-11',
    name: 'Webcam Shows Black Screen on Windows 11',
    description: 'Fix webcam shows black screen on windows 11 with clear steps for Windows 11 covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Webcam Shows Black Screen on Windows 11', path: '/issues/webcam-black-screen-windows-11' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Webcam Shows Black Screen on Windows 11</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Webcam turns on but only shows a black screen on Windows 11. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>

          <IssueDiagnostic device="webcam" mode="defer" />

          <IssueLinksPanel issue={{ slug: "webcam-black-screen-windows-11", deviceType: "webcam", platform: "Windows 11", title: "Webcam Shows Black Screen on Windows 11" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Webcam turns on but only shows a black screen"
              platform="Windows 11"
              deviceType="webcam"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'webcam', platform: "Windows 11", slug: 'webcam-black-screen-windows-11' }} />
