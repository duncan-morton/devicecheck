...
const issuePath = '/issues/keyboard-function-keys-not-working-windows'
const alternates = {
...
export const metadata: Metadata = { ...genMeta({
  title: 'Function Keys Not Working on Windows - Complete Fix Guide',
  description: 'Fix function keys not working on windows. Step-by-step troubleshooting guide covering Windows settings, permissions, drivers, and solutions for f1–f12 keys inactive or reversed.',
  path: issuePath,
  keywords: ["function keys not working windows","f keys not responding","fn key windows problem"]
}), alternates }

const faqs = [
  {
    "question": "Why is function keys not working on windows?",
    "answer": "F1–F12 keys inactive or reversed. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix function keys not working on windows?",
  ...
  {
    "question": "What causes function keys not working on windows?",
  ...
  {
    "question": "Can I fix function keys not working on windows without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Function Keys Not Working on Windows - Complete Fix Guide',
    'Fix function keys not working on windows. Step-by-step troubleshooting guide.',
    '/issues/keyboard-function-keys-not-working-windows',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Windows device issues","path":"/hubs/windows-device-issues"},{"name":"Function Keys Not Working on Windows","path":"/issues/keyboard-function-keys-not-working-windows"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/keyboard-function-keys-not-working-windows',
    name: 'Function Keys Not Working on Windows',
    description: 'Fix function keys not working on windows with clear steps for Windows covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Function Keys Not Working on Windows', path: '/issues/keyboard-function-keys-not-working-windows' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Function Keys Not Working on Windows</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              F1–F12 keys inactive or reversed on Windows. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>
          ...
          <IssueLinksPanel issue={{ slug: "keyboard-function-keys-not-working-windows", deviceType: "keyboard", platform: "Windows", title: "Function Keys Not Working on Windows" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="F1–F12 keys inactive or reversed"
              platform="Windows"
              deviceType="keyboard"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'keyboard', platform: "Windows", slug: 'keyboard-function-keys-not-working-windows' }} />
