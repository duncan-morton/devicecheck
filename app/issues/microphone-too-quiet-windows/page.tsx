...
const issuePath = '/issues/microphone-too-quiet-windows'
const alternates = {
...
export const metadata: Metadata = { ...genMeta({
  title: 'Microphone Too Quiet on Windows - Complete Fix Guide',
  description: 'Fix microphone too quiet on windows. Step-by-step troubleshooting guide covering Windows settings, permissions, drivers, and solutions for low microphone volume.',
  path: issuePath,
  keywords: ["microphone too quiet windows","mic volume low fix","quiet microphone windows"]
}), alternates }

const faqs = [
  {
    "question": "Why is microphone too quiet on windows?",
    "answer": "Low microphone volume. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix microphone too quiet on windows?",
  ...
  {
    "question": "What causes microphone too quiet on windows?",
  ...
  {
    "question": "Can I fix microphone too quiet on windows without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Microphone Too Quiet on Windows - Complete Fix Guide',
    'Fix microphone too quiet on windows. Step-by-step troubleshooting guide.',
    '/issues/microphone-too-quiet-windows',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Windows device issues","path":"/hubs/windows-device-issues"},{"name":"Microphone Too Quiet on Windows","path":"/issues/microphone-too-quiet-windows"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/microphone-too-quiet-windows',
    name: 'Microphone Too Quiet on Windows',
    description: 'Fix microphone too quiet on windows with clear steps for Windows covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Microphone Too Quiet on Windows', path: '/issues/microphone-too-quiet-windows' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Microphone Too Quiet on Windows</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Low microphone volume on Windows. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>
          ...
          <IssueLinksPanel issue={{ slug: "microphone-too-quiet-windows", deviceType: "mic", platform: "Windows", title: "Microphone Too Quiet on Windows" }} allIssues={issuesData} />
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'mic', platform: "Windows", slug: 'microphone-too-quiet-windows' }} />
