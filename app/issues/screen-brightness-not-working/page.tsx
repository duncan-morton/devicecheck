...
const issuePath = '/issues/screen-brightness-not-working'
export const metadata: Metadata = { ...genMeta({
  title: 'Screen Brightness Not Working - Complete Fix Guide',
  description: 'Fix screen brightness not working. Step-by-step troubleshooting guide covering All settings, permissions, drivers, and solutions for brightness controls not responding.',
  path: issuePath,
  keywords: ["screen brightness not working","brightness keys fix","monitor brightness issue"]
}) }

const faqs = [
  {
    "question": "Why is screen brightness not working?",
    "answer": "Brightness controls not responding. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix screen brightness not working?",
  ...
  {
    "question": "What causes screen brightness not working?",
  ...
  {
    "question": "Can I fix screen brightness not working without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Screen Brightness Not Working - Complete Fix Guide',
    'Fix screen brightness not working. Step-by-step troubleshooting guide.',
    '/issues/screen-brightness-not-working',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Screen Brightness Not Working","path":"/issues/screen-brightness-not-working"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/screen-brightness-not-working',
    name: 'Screen Brightness Not Working',
    description: 'Fix screen brightness not working with clear steps for All covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Screen Brightness Not Working', path: '/issues/screen-brightness-not-working' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Screen Brightness Not Working</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Brightness controls not responding on All. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Brightness controls not responding"
              platform="All"
              deviceType="screen"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'screen', platform: "All", slug: 'screen-brightness-not-working' }} />
