...
const faqs = [
  {
    "question": "Why is screen scaling not working?",
    "answer": "Resolution or scaling settings ignored. This usually happens due to permission settings, driver issues, or conflicts with other applications. Start by checking system permissions and ensuring the correct device is selected."
  },
  {
    "question": "How do I fix screen scaling not working?",
  ...
  {
    "question": "What causes screen scaling not working?",
  ...
  {
    "question": "Can I fix screen scaling not working without technical knowledge?",
...
export default function IssuePage() {
  const articleSchema = generateArticleSchema(
    'Screen Scaling Not Working - Complete Fix Guide',
    'Fix screen scaling not working. Step-by-step troubleshooting guide.',
    '/issues/screen-not-scaling-correctly',
    new Date().toISOString(),
    new Date().toISOString()
  )

  const breadcrumbItems = [{"name":"Home","path":"/"},{"name":"Issues","path":"/issues"},{"name":"Screen Scaling Not Working","path":"/issues/screen-not-scaling-correctly"}]
  const breadcrumbs = generateBreadcrumbListSchema(breadcrumbItems)

  const faqSchema = generateFAQPageSchema(faqs)
  const howToSchema = generateHowToSchema({
    url: 'https://devicecheck.io/issues/screen-not-scaling-correctly',
    name: 'Screen Scaling Not Working',
    description: 'Fix screen scaling not working with clear steps for All covering permissions, device selection, and drivers.',
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          <Breadcrumbs items={[
            ...
            { name: 'Screen Scaling Not Working', path: '/issues/screen-not-scaling-correctly' }
          ]} />
          
          <div className="mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Screen Scaling Not Working</h1>
            <p className="text-xl text-gray-600 max-w-3xl">
              Resolution or scaling settings ignored on All. This guide covers all solutions, from permissions to driver updates.
            </p>
          </div>
          ...
          <article id="article-content" className="prose prose-slate max-w-none bg-white p-8 md:p-12 rounded-2xl border border-gray-200 mt-8">
            <QuickAnswerBox 
              problem="Resolution or scaling settings ignored"
              platform="All"
              deviceType="screen"
            />
            ...
            <TroubleshootingMatrix issue={{ deviceType: 'screen', platform: "All", slug: 'screen-not-scaling-correctly' }} />
